 ...
 * CCICCacheData.cp
 *
 * $Header: /cvs/pismere/pismere/athena/auth/krbcc/src/CCacheLib/Sources/CCICCacheData.cp,v 1.6 2002/08/16 21:37:17 pilawa Exp $
 */
...
CCICCacheData::SetPrincipal (
	...
	// changed by pilawa to not break under .NET	std::remove_if (mCredentials.begin (), mCredentials.end (), std::bind2nd(std::equal_to<CCICredentialsData*>(), nil)); 
	SharedCredentialsVector::iterator	newEnd = 
				std::remove_if (mCredentials.begin (), mCredentials.end (), std::bind2nd(std::equal_to<CCICredentialsData*>(), (CCICredentialsData*) nil));
