...
int readlink(char *path, char *buf, int buffers)
{
	return -1;
}
...
static long get_cellconfig_callback(void *cellconfig, 
...
long GetLocalCell(struct afsconf_dir **pconfigdir, char *local_cell)
{
	...
	return cm_GetRootCellName(local_cell);
#else
...
struct afsconf_cell **pcellconfig)
{
	...
	strcpy((*pcellconfig)->name, cell);

	return cm_SearchCell(cell, get_cellconfig_callback, NULL,
		(void*) *pcellconfig);
#else
