    ...
    char *fn;                   /* Filename to get keys from */
{
    ...
    if (krb_ap_req_debug)
        klog("ticket->length: %d",tkt->length);

#ifndef NOENCRYPTION
    /* Decrypt and take apart ticket */
#endif

    if (k_decomp_ticket(tkt,&ad->k_flags,ad->pname,ad->pinst,ad->prealm,
                      &(ad->address),ad->session, &(ad->life),
                      &(ad->time_sec),sname,iname,ky,serv_key))
        return(RD_AP_UNDEC);

    if (krb_ap_req_debug) {
        klog("Ticket Contents.");
        klog(" Aname:   %s.%s",ad->pname,
            ((int)*(ad->prealm) ? ad->prealm : "Athena"));
        klog(" Service: %s%s%s",sname,((int)*iname ? "." : ""),iname);
    }
...
#define check_ptr() if ((ptr - (char *) req_id->dat) > req_id->length) return(RD_AP_MODIFIED);
    ...
    if (krb_ap_req_debug)
        klog("Pname:   %s %s",ad->pname,r_aname);
    if (strcmp(ad->pname,r_aname) != 0)
        return(RD_AP_INCON);
    if (strcmp(ad->pinst,r_inst) != 0)
        return(RD_AP_INCON);
    if (krb_ap_req_debug)
        klog("Realm:   %s %s",ad->prealm,r_realm);
    if ((strcmp(ad->prealm,r_realm) != 0))
        return(RD_AP_INCON);

    if (krb_ap_req_debug)
        klog("Address: %d %d",ad->address,from_addr);
    ...
    if (krb_ap_req_debug)
        klog("Time: %d Issue Date: %d Diff: %d Life %x",
    ...
    if (t_local.tv_sec < ad->time_sec) {
        ...
        return(RD_AP_EXP);
#else
    else if (krb_life_to_time(ad->time_sec, ad->life)
	     < t_local.tv_sec + CLOCK_SKEW) {
        /*
