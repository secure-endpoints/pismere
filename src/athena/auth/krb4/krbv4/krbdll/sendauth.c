...
krb_sendauth(long options,         /* bit-pattern of options             */
    ...
    LPSTR realm,                   /* service name, instance, realm  */
    LONG checksum,                 /* checksum to include in request     */
...
#endif
{
    ...
    /* hey buddy, do we have a valid socket? */

    if( SOCKET_ERROR == getsockopt( fd, SOL_SOCKET, SO_ERROR, &cRet, &len) ){
	len = WSAGetLastError(); // good chance it is 10038, not a socket
	return(len);
    }
