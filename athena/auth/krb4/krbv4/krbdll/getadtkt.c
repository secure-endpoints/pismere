...
int     lifetime;
{
    ...
    if ((kerror = krb_get_cred("krbtgt",realm,lrealm,&cr)) != KSUCCESS) {  
   	...
   	if ((strncmp(realm, lrealm, REALM_SZ)) == 0){
   	    ...
   	    if (krb_debug){
                kdebug("realm == lrealm in get_ad_tkt \n");
   	    }
            return(AD_NOTGT);
   	}
        else{
   		...
   		if (krb_debug){
                    kdebug("Problem with get_ad_tkt( krbtgt, %s, %s, %ld \n",
                           realm, lrealm, lifetime );
   		}
                return(kerror);
   	    }
   	    if ((kerror = krb_get_cred("krbtgt",realm,lrealm,&cr)) != KSUCCESS) {
   		...
   		if (krb_debug){
                    kdebug("Problem with krb_get_cred( krbtgt, %s, %s, &cr \n",
                           realm, lrealm);
   		}
    ...
    *(pkt->dat+(pkt->length)++) = (char) lifetime;
    ...
    if (krb_debug)  kdebug("About to do decryption ...");
